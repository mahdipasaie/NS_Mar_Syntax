...
from fenics import (
    ...
    near, TrialFunction, LogLevel, set_log_level, sqrt
)
...
grid_spacing_x = 0.2e-3  # 0.1 mm converted to meters
grid_spacing_y = 0.2e-3  # 0.1 mm converted to meters
...
mesh  = refine_mesh_near_corners( mesh, 0.1e-3, Domain  )
...
def F2(u_answer, u_prev, p_answer, T_answer, v_test, dt, rho1, n_v, mu1, gamma, alpha1, ds1, dx1):
    ...
    T_answer: dolfin.Function
        The current approximation of the temperature field in the mixed function space.
...
def F3(T_answer, T_prev, u_answer, s_test, dt, rho1, Cp1, K1):
    """
    Define the weak form of the energy equation for the coupled Navier-Stokes and heat transfer problem.

    Args:
    T_answer: fenics.Function
        The current approximation of the temperature field in the mixed function space.
    T_prev: fenics.Function
        The temperature field from the previous time step.
    u_answer: fenics.Function
        The current approximation of the velocity field in the mixed function space.
    s_test: fenics.TestFunction
        The test function for temperature in the mixed function space.
    dt: float
        The time step for the transient simulation.
    rho1: float
        The density of the fluid.
    Cp1: float
        The specific heat capacity at constant pressure of the fluid.
    K1: float
        The thermal conductivity of the fluid.

    Returns:
    ufl.Form
        The weak form of the energy equation suitable for FEniCS assembly.
    """
...
def Define_Boundary_Condition(W, Domain, T_LEFT, T_RIGHT  ) : 
    ...
    zero_pressure_point = Point( (X0 + X1) /2,  (Y0+Y1)/2 )
...
file = fe.XDMFFile("NS_Syntax.xdmf" ) # File Name To Save #
